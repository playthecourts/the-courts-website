...
export default async function CoachAthletePage(props: PageProps<"/coach/athletes/[id]">) {
  ...
  const athlete = await prisma.athlete.findUniqueOrThrow({
    ...
    select: {
      ...
      // The last two are reachable only through the audited reveal action;
      // the first two aren't operationally needed on this screen.
      family: {
        select: {
          id: true,
          guardians: {
  ...
  const [recent, upcoming, notes, pastDue, plan] = await Promise.all([
    ...
    <div>
      ...
      <Card className="mb-4 px-4 py-4">
        <div className="flex items-center gap-3">
          <Avatar initials={initials(athlete.firstName, athlete.lastName)} />
